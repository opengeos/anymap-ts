import{m as ee,W as te,p as ne,C as ie}from"./polygon-utils-DnOoNwHd.js";import{K as L,L as oe,H as j,I as se,J as N,U as D,a6 as T,a7 as re,a5 as ae,D as le,a8 as ce}from"./mapbox-overlay-CcF19Pc8.js";import{G as ue,g as de}from"./geometry-B-6Jnco5.js";class ge extends ue{constructor(e){const{indices:t,attributes:n}=fe(e);super({...e,indices:t,attributes:n})}}function fe(r){const{radius:e,height:t=1,nradial:n=10}=r;let{vertices:i}=r;i&&(L.assert(i.length>=n),i=i.flatMap(g=>[g[0],g[1]]),ee(i,te.COUNTER_CLOCKWISE));const o=t>0,s=n+1,a=o?s*3+1:n,l=Math.PI*2/n,d=new Uint16Array(o?n*3*2:0),c=new Float32Array(a*3),f=new Float32Array(a*3);let u=0;if(o){for(let g=0;g<s;g++){const h=g*l,p=g%n,m=Math.sin(h),b=Math.cos(h);for(let v=0;v<2;v++)c[u+0]=i?i[p*2]:b*e,c[u+1]=i?i[p*2+1]:m*e,c[u+2]=(1/2-v)*t,f[u+0]=i?i[p*2]:b,f[u+1]=i?i[p*2+1]:m,u+=3}c[u+0]=c[u-3],c[u+1]=c[u-2],c[u+2]=c[u-1],u+=3}for(let g=o?0:1;g<s;g++){const h=Math.floor(g/2)*Math.sign(.5-g%2),p=h*l,m=(h+n)%n,b=Math.sin(p),v=Math.cos(p);c[u+0]=i?i[m*2]:v*e,c[u+1]=i?i[m*2+1]:b*e,c[u+2]=t/2,f[u+2]=1,u+=3}if(o){let g=0;for(let h=0;h<n;h++)d[g++]=h*2+0,d[g++]=h*2+2,d[g++]=h*2+0,d[g++]=h*2+1,d[g++]=h*2+1,d[g++]=h*2+3}return{indices:d,attributes:{POSITION:{size:3,value:c},NORMAL:{size:3,value:f}}}}const R=`uniform columnUniforms {
  float radius;
  float angle;
  vec2 offset;
  bool extruded;
